...
app.post(controllerPath, (request, response) => {
    ...
    if (request.body && request.body.id && request.body.message) {  // Validate that the incoming payload contains: [id] and [message]... if not then respond with invalid message and coordinating error.
        if (counter.consumedValues.some(id => id == request.body.id)) {
            ...
            response.status(500).send(JSON.stringify({error: 'Invalid value for [id].  Only unique [id] value is acceptable.'}))
        }
        else {
            ...
            response.send(JSON.stringify({count: counter.currentIncrementValue}));
            
        }
    }
    
    response.status(405).send(JSON.stringify({error: 'Values presented in the payload do not match the expected request structure.  The JSON request should fit the following structure:   { "id": "123", "message": "hello world" }'}));
})
